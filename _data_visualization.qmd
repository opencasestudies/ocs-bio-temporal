...
|---|---|---|---|---|
| K-selection figure | `figure_01_k_selection_source.csv` | How candidate model ranks balance stability, temporal resolution, and goal-aligned evidence. | Which value of `K` provides an appropriate CoGAPS model for this case-study goal? | It does not identify a biological program by itself. |
...
| Pattern usage by broad cell type | `figure_04_pattern_usage_by_cell_type_matrix.csv` | Mean pattern usage across broad PBMC identities. | Which programs reflect PBMC identity, dynamic infection activity, or both? | It does not show whether a pattern changes over time within the same cell type. |
...
This figure is a visual preview of Research Question 1. The top panel asks whether a model rank is stable across seeds while still resolving temporal structure. The bottom panel summarizes a goal-aligned score that combines stability, low redundancy, temporal signal, and IFN-candidate evidence.
