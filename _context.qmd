...
That difference matters because single-cell data are noisy, sparse, and high-dimensional. Rather than treating one fitted factorization as automatically meaningful, CoGAPS supports diagnostics that help you decide whether a run is stable enough to interpret. In other words, the model output is a starting point for evidence-based interpretation, not a final answer by itself.
...
Later in the case study, you will inspect precomputed rank-sweep evidence and decide which value of `K` is appropriate for the main analysis goal. The evidence will include stability, redundancy, temporal signal, interpretability, and biological plausibility. This section does not reveal the selected rank because choosing `K` is one of the research questions you will answer.
...
::::
::::{.inner_block}
- **DENV-1:** Dengue virus serotype 1, the dengue virus serotype studied in the source experimental and natural infection cohorts.
- **Serotype:** A subtype of a virus distinguished by how the immune system recognizes it.
...
- **Experimental infection cohort:** A study group sampled on a planned infection timeline, allowing responses to be compared across aligned experimental days.
- **Natural infection cohort:** A study group infected outside the study setting and sampled during real-world illness or recovery.
- **Interferon:** An immune signaling protein that helps coordinate antiviral responses.
...
- **Latent transcriptional program:** An inferred gene-expression pattern shared across genes and cells, rather than a directly measured variable.
