...
Data Import selected the `K = 10` {{< glossary "CoGAPS" >}} model and loaded its compact {{< glossary "Selected-model outputs" display="selected-model files" >}}. Before you use any numbered {{< glossary "Pattern label" display="pattern label" >}} as biological evidence, pause to check what those files can support. In this section, you will inspect the {{< glossary "Balanced discovery set" display="prepared discovery set" >}}, the broad PBMC mixture, and the model-evidence tables that later sections use to evaluate candidate temporal programs.
...
| Broad PBMC composition | Does the {{< glossary "Cell composition" display="cell composition" >}} of the balanced discovery set vary across days? | A pattern can look temporal if the mixture of immune-cell types changes over time. |
...
<details>
<summary>Optional: see how the composition figure was created</summary>
...
Finally, the code labels the axes, places the legend below the plot, and saves the same figure as both `PNG` and `SVG` files for the case study.
...
save_figure(fig, outdir, "figure_02_discovery_set_composition", written)
```
...
1. Which unit of evidence should carry the temporal claim, and why?
2. How could cell composition make a pattern look temporal?
3. Why are top genes useful but not sufficient for naming a CoGAPS pattern?
