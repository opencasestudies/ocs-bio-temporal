...
1. read the route map to see where the included files came from;
...
- **External reproduction archive:** the larger Zenodo archive. Mount it read-only when you want to rerun downstream model, directionality, or audit steps without rebuilding every source file.
...
```{r}
#| label: full-reproduction-analysis-scripts
#| echo: false
#| results: asis
script_paths <- c(
