...
| Selected model outputs | Pattern scores, gene loadings, top genes, diagnostics, and research-question summary tables from the selected model. | Standard path | Data Import, Visualization, and Analysis use these files directly. |
...
Second, the model-rank evidence and selected model results are prepared for interpretation. The rank-selection summaries let you evaluate the modeling decision, and the selected model outputs provide the pattern scores, gene loadings, diagnostics, and smaller tables used in the standard path through the case study. **Data Visualization** shows how those tables become figures. **Data Analysis** uses them to answer the research questions.

The full raw, preprocessed, model-object, and checkpoint files are too large for the standard path, so they are stored in the Zenodo reproduction archive. The archive is the optional large-file layer for full reproduction; the case-study repository still supplies the text, scripts, manifests, and smaller files needed to inspect the selected model, reproduce the main summaries, and follow the interpretation.
...
| Pattern summary tables | One CoGAPS pattern. | Ranking temporal variation, identity/activity structure, and IFN association. | Pattern labels are model labels, not biological names. |
...
|---|---|---|
| `Pattern1` to `Pattern10` | CoGAPS pattern scores for the included selected model. | Larger values mean stronger activity of that pattern in a cell or group. |
