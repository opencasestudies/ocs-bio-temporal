...
The K-selection sweep is not rerun in either path. You will inspect precomputed rank-selection evidence and selected code excerpts when you evaluate why `K = 10` is used for the main analysis.
...
| K-selection evidence layer | Understand how the CoGAPS rank was selected. | Included K-selection tables and figures; documented sweep scripts. | Inspect precomputed summaries and selected code excerpts, but do not rerun the sweep. |
...
Second, download and extract the Zenodo reproduction archive into a folder outside the Git repository. Do not commit those files to GitHub. The archive is a large-file layer for full reproduction: it can contain original source files, preprocessed H5AD files, selected-model objects, checkpoint files, and sweep summaries.

During draft review, the Zenodo record is listed as DOI pending publication in the front matter. Once the record is public, download the reproduction archive into a local folder such as `$HOME/cs5_reproduction_archive`.
...
1. Rebuild interpretation-layer tables:
   `scripts/reproduction/cs5_build_k10_interpretation_layer.py`.
1. Rebuild directionality summaries:
...
The Docker image installs the packages used by this case study. The code chunks below load the packages used for the standard path. You may work in R or Python; the two tabs show alternate implementations of the same ideas.
...
data.frame(
  ...
  available = c(
    ...
    dir.exists(here::here("data", "processed", "selected_model_k10")),
...
pd.DataFrame({
    ...
    "available": [
        ...
        Path("data/processed/selected_model_k10").is_dir(),
...
When a function is introduced for the first time, the prose uses the `package::function()` form so you can see which package provides it. Next, you will import the small selected-model files that carry the CoGAPS results through the rest of the case study.
