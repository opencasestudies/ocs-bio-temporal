...
The K-selected files are exported outputs from a CoGAPS model fit to the prepared balanced experimental discovery matrix. The model fit produces two core kinds of information: **gene loadings**, which show which genes help define each pattern, and **cell scores**, which show how strongly each cell uses each pattern. The smaller tables you import here are derived from those outputs so you can inspect the selected model without rerunning CoGAPS.

Some files summarize the model fit itself. The pattern summary and top-gene table help you see what patterns were learned and which genes contribute to them. Other files summarize the run and its diagnostics. The metrics file records settings such as `K`, seed, iterations, runtime, and thread count, while the trace file records diagnostic values saved during the run. You will import these diagnostic outputs now so the files are ready, but you will interpret them later in **Data Analysis** before treating any pattern as biological evidence.

For now, the goal is practical: load the files that belong to the selected `K = 10` run and confirm that they have the expected structure. The biological interpretation comes later, after the model outputs are connected to subject, timepoint, cell-type, top-gene, directionality, and diagnostic evidence.
