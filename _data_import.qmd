...
In this section, you will import the files that enter the analysis environment. Import means reading files into R or Python objects so you can inspect them later. It does not yet mean filtering cells, summarizing patterns, making figures, or answering the research questions.
...
There are two ways to use this section. In the **standard path**, you import the small selected-model files that are already included with the case study. In the **full reproduction path**, you also import the original source 10x Genomics files after downloading the large Zenodo and GEO files. The preprocessing decisions that happen after source-file import are covered in **Data Wrangling and Preprocessing**.

The boundary between sections is important:

| Section | What happens there | Main learner action |
|---|---|---|
| Data Import | Read files into R or Python objects. | Check that expected files exist and load correctly. |
| Data Exploration | Inspect what the imported files represent. | Ask whether subjects, timepoints, and broad cell types are represented as expected. |
| Data Wrangling and Preprocessing | Prepare and reshape data for modeling or summaries. | Understand filtering, normalization, annotation, balancing, and subject-level summaries. |
| Data Analysis | Use the prepared evidence to answer the research questions. | Interpret model selection, diagnostics, temporal programs, top genes, and directionality. |

The standard path imports four selected-model file families:
...
Before importing model results, inspect the file inventory. The inventory is a `JSON` file that records which small files are included locally and which large files are kept in the Zenodo reproduction archive. This check helps you understand why some files can be loaded immediately while others require the full reproduction setup.
...
## Standard Path: Import Selected-Model Results
...
r_import_plan <- data.frame(
  ...
  used_later_in = c(
    "Data Analysis: temporal programs",
